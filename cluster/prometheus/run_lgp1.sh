...
[[ "$RUN" == "$ROOT/experiments/local-goal-proposals-20260918/"* ]] || exit 2
[[ ! -e "$RUN/$TASK" ]] || exit 2
(cd "$SRC" && sha256sum --check --quiet LGP1-SOURCE-MANIFEST.sha256)
...
apptainer exec --cleanenv "${GPU_ARGS[@]}" --bind "$ROOT:$ROOT:ro" --bind "$RUN:$RUN:rw" "$IMAGE" env \
 ...
 "$ENV/bin/python" -B "$SRC/cluster/prometheus/lgp1_worker.py" \
 --source "$SRC" --approval "$APPROVAL" --run "$RUN" --task "$TASK"
