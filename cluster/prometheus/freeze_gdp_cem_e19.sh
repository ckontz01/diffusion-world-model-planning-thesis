...
STAGED_ROOT=${1:?usage: freeze_gdp_cem_e19.sh STAGED_ROOT OFFICIAL_SAGE OUTPUT_PARENT}
OFFICIAL_SAGE=${2:?usage: freeze_gdp_cem_e19.sh STAGED_ROOT OFFICIAL_SAGE OUTPUT_PARENT}
OUTPUT_PARENT=${3:?usage: freeze_gdp_cem_e19.sh STAGED_ROOT OFFICIAL_SAGE OUTPUT_PARENT}
...
export PYTHONPATH=${BUILD}:${BUILD}/official-sage
...
"${ENV_DIR}/bin/python" - "${BUILD}" "${ENV_DIR}" "${COMMIT}" "${TREE}" "${PROTOCOL}" <<'PY'
...
payload = {
    ...
    "protocol_sha256": digest(root / sys.argv[5]),
