...
[[ "$RUN" == "$ROOT/experiments/local-goal-source-replication-20260920/"* && ! -e "$RUN/$TASK" ]] || exit 2
(cd "$SRC" && sha256sum --check --quiet LGP-RB2-SOURCE-MANIFEST.sha256)
...
apptainer exec --cleanenv "${GPU_ARGS[@]}" --bind "$ROOT:$ROOT:ro" --bind "$RUN:$RUN:rw" "$IMAGE" env \
 ...
 "$ENV/bin/python" -B "$SRC/cluster/prometheus/lgprb2_worker.py" --source "$SRC" --approval "$APPROVAL" --run "$RUN" --task "$TASK"
