...
def main():
    ...
    existing=c.old.size(a.run);soft=spec['seconds']-(30 if spec['kind']=='evaluation' else 60)
    def guard():
        c.require(time.monotonic()-began<soft,'Worker preservation margin')
        current=c.old.size(out)
        c.require(current< (2*c.CAPS['episode_bytes'] if spec['kind']!='analysis' else 200_000_000),'Worker output cap')
    ...
    try:
        c.old.authenticate_inputs(a.source,payload=False);guard()
        if spec['gpu']:
            import torch
            ...
            from lgprb1_evaluate import evaluate,compatibility
            result=(evaluate if spec['kind']=='evaluation' else compatibility)(a.source,out,spec,guard)
        else:
            c.require(not os.environ.get('CUDA_VISIBLE_DEVICES'),'CPU-only final analysis')
            from lgprb1_analysis import aggregate
        ...
        for key in ['episodes','models_unchanged','first_decisions','physical_actions','passed','setup_seconds']:
