...
staging=${output_parent}/.gdp-post-e14-boundary-staging-r8-20260825
test ! -e "${staging}"
mkdir -p "${staging}"
cp -a "${BASE}/." "${staging}/"
chmod -R u+w "${staging}"
...
printf '%s\n' \
  ...
  'checks=python_compile,shell_syntax,boundary_mask_semantics,released_float32_environment_to_planner_transform,strict_and_tolerant_legal_bounds,weighted_axis_aggregation,equal_condition_aggregation,full_row_distribution,equal_task_seed_aggregation' \
