...
staging=${output_parent}/.gdp-post-e14-boundary-staging-20260825
test ! -e "${staging}"
mkdir -p "${staging}"
cp -a "${BASE}/." "${staging}/"
chmod -R u+w "${staging}"
...
printf '%s\n' \
  ...
  'checks=python_compile,shell_syntax,boundary_mask_semantics,weighted_axis_aggregation,equal_condition_aggregation' \
