#!/usr/bin/env bash
set -euo pipefail
source_root=${1:?usage: freeze_gdp_cem_e12_stage_b_finalize.sh STAGED_SOURCE OUTPUT_PARENT}
output_parent=${2:?usage: freeze_gdp_cem_e12_stage_b_finalize.sh STAGED_SOURCE OUTPUT_PARENT}
...
staging=${output_parent}/.gdp-cem-e12-stage-b-finalize-staging-20260820
test ! -e "${staging}"
mkdir -p "${staging}"
files=(
  ACID-ALTERNATIVE-E12-PRISM-MATCHED-UNTOUCHED-D4-PROTOCOL-2026-08-20.md
  gdp_cem_e11_specs.py gdp_cem_e12_specs.py
  collect_gdp_cem_e12_artifacts.py run_gdp_cem_e12_stage_b_collect.slurm
  submit_gdp_cem_e12_stage_b_finalize.sh freeze_gdp_cem_e12_stage_b_finalize.sh
)
...
destination=${output_parent}/gdp-cem-e12-stage-b-finalize-${manifest_hash:0:16}
test ! -e "${destination}"
