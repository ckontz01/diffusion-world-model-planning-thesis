...
def validate(data,expected=6000,witnesses=32):
    ...
    assert m['namespace']=='final-20260906-primary-v1'
    ...
    for i,ref in enumerate(m['records']):
        ...
        with np.load(data/ref['file'],allow_pickle=False) as f:
            s=f['states'].copy();a=f['actions'].copy();d=f['dynamics'].copy()
        assert s.shape==(151,7) and a.shape==(150,2) and d.shape==(151,11)
        ...
        if i in set(np.linspace(0,expected-1,min(witnesses,expected),dtype=int)):
            for h in (75,150):
                ...
                try:
                    env.reset(options={OPTION:{'state':s[0],'goal_state':s[h]}})
