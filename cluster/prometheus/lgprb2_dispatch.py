...
class Slurm(OldSlurm):
    def submit(self,spec):
        args=['sbatch','--parsable','--no-requeue','--account=superworld','--cpus-per-task=4',
            ...
            '--job-name=lgprb2-'+spec['name'],'--output='+str(self.root/('slurm-'+spec['name']+'-%j.out')),
            '--error='+str(self.root/('slurm-'+spec['name']+'-%j.err'))]
        if spec['gpu']:args+=['--gres=gpu:1']
        args += [str(self.source/'cluster/prometheus/run_lgprb2.sh'),str(self.source),str(self.approval),str(self.root),spec['name']]
...
def execute(specs,scheduler,record,check,stage,bytes_check):
    ...
    for i,spec in enumerate(specs):
        ...
        c.require(sum(c.CAPS['main_job_bytes'] if s['gpu'] else c.CAPS['analysis_bytes'] for s in specs)<=c.CAPS['worker_bytes'],'Full payload reservations')
        if i==4:stage('technical',completed)
        ...
        check(spec)
    return dict(completed=completed,gpu_seconds=gpu,cpu_seconds=cpu,new_jobs=1537,new_main_episodes=12288,reused_main_episodes=0)
...
def main():
    ...
    old=subprocess.check_output(['sacct','-X','-n','-P','--starttime=2026-09-20','--format=JobName%100'],text=True)
    c.require(not any(s.startswith('lgprb2-') for s in old.splitlines()),'Prior allocation; stop for reconciliation')
    c.verify_models(a.source)
    a.run.mkdir(parents=True,exist_ok=False);c.write(a.run/'APPROVAL.json',approval)
    with (a.run/'PRE-EVALUATION-FREEZE.json').open('xb') as f:f.write((c.rb1.OLD_RUN/'PRE-EVALUATION-FREEZE.json').read_bytes())
    c.write(a.run/'MODELS-VERIFIED.json',dict(source_sha256=c.RB1_SOURCE_SHA,freeze_sha256=c.FREEZE_SHA,reused_episodes=0,unix=time.time()))
    ...
    def stage(name,rows):
        if name=='technical':
            c.require(len(rows)==4,'First four included main jobs')
            c.write(a.run/'TECHNICAL-TRANCHE-PASSED.json',dict(unix=time.time(),seals={s['name']:c.sha(a.run/s['name']/'sha256.txt') for s in specs[:4]},scientific_selection=False))
        else:
            c.require(len(rows)==1536,'All fixed main allocations')
            c.write(a.run/'PRE-ANALYSIS-ACCOUNTING.json',dict(jobs=rows,reused_historical_episodes=0,new_optimizer_updates=0))
    try:
