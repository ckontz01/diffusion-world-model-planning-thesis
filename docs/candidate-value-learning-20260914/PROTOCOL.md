...
Status: **proposed, outcome-informed method development; NOT approved to launch**.
...
artifacts and 40 GB free on the external backup SSD. Collection is likely about
...
Source identity authentication remains pending; a synthetic identity test is not
a certificate about the real collection. Do not load broad outcome-bearing
metadata unnecessarily to establish identity.
...
assign each branch's explicit proposal generators the deterministic seed
`seed('tail', reference,H,t,draw)` for draws 0 and 1; draw is **not** part of the
...
negative. A verified planner failure after a technically valid setup is a policy
failure (zero unless success already occurred); record category and stop further
dispatch pending diagnosis, without deleting it or substituting a distance label.
...
- Current LeWM latent 192, goal latent 192, candidate predicted terminal latent
  ...
  three, including the LeWM prediction; no new fitted latent scaler.
...
Train/freeze all four models before opening validation labels. Validate within
...
point difference is **positive vs continuation**; MLP Brier is no worse than the
train-constant predictor under the same hierarchical weights. No claim of
...
0. Finish the thin fresh-driver collection adapter and approval-gated dispatcher
   against this exact protocol; freeze their complete dependency closure, protocol,
   source manifest, IDs, schemas and execution contract **before any new outcome**.
   The current implemented core/hook is not a complete production dispatch system.
   Verify selected source identity and pinned bytes without opening protected data.
...
1. First four training references (allocation order), both horizons: eight of the
   ...
   <=16GiB sampled host RSS and <=20GB full-study storage projection; if exceeded,
...
2. Complete remaining training jobs; authenticate seals and apply sparse gate.
   Train the four fixed models on train labels only, freeze weights and receipts.
