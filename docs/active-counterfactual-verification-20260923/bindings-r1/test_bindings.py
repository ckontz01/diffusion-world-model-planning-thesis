...
FIXTURE=c.BASE/'bindings-v1/completion-tests-v1'  # Reuse preserved artificial fits; never rerun them.
...
class Bindings(unittest.TestCase):
    ...
    def test_auth_before_import_or_payload(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            a={'schema':'ACV0-recovery-r1','recovery':{},'authorized':False,'instruction':'', 'package_sha256':'missing',
    ...
    def test_same_samples_distinct_actual_future_inputs(self):
        ...
        models=load_models(FIXTURE);selector=Selector(*models)
    ...
    def test_full_independent_aggregation_and_footprint(self):
        ...
        root=FIXTURE;models=load_models(root)
    ...
    def test_grid_gates_reservations_and_no_retry(self):
        ...
        self.assertEqual(sum(j['seconds'] for j in jobs if j['gpu'])+23,220763)
        ...
        self.assertEqual(gates,[('technical',2),('models',82),('analysis',338)]);self.assertEqual(r['new_attempts'],339)
        self.assertEqual(r['campaign_attempts'],340);self.assertEqual(r['gpu_seconds'],220763)
        for kind in ('fail','ambiguous'):
            ...
            if kind=='fail':self.assertEqual(events[-1]['gpu_seconds'],3563)
