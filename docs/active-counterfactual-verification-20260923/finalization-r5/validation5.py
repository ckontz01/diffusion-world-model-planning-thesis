...
def accept(ctx,result):
    r.verify_baseline(ctx);guard(ctx.recovery_context)
    ...
    snapshot=r.read(ctx.recovery_context.control/'FINAL-SCHEDULER.json');alloc=snapshot['allocations']
