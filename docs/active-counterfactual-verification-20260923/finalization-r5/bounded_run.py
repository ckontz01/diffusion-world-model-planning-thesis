...
if __name__ == '__main__':
    ...
    publication = common.REPO / 'docs/active-counterfactual-verification-publication-20260923/finalization-r5'
    ...
    prior=sum((common.lines(root.parent/folder/'ATTEMPTS.jsonl') for folder in ('control-r2','control-r3','control-r4')),[])
    ...
    result.update(label=label, state='finished', wall_seconds=time.monotonic()-start,
                  ...
                  output_bytes=sum(p.stat().st_size for folder in (root,publication,root.parent/'control-r2',publication.parent/'continuation-r2',root.parent/'control-r3',publication.parent/'continuation-r3',root.parent/'control-r4',publication.parent/'continuation-r4') for p in folder.rglob('*') if p.is_file()), gpu_allocations=0)
