...
def main():
    ...
    def timeout(*_):raise TimeoutError('60-second preservation margin; no retry')
    signal.signal(signal.SIGALRM,timeout);signal.signal(signal.SIGTERM,timeout)
    signal.alarm(spec['seconds']-(120 if spec['stage']=='collection' else 60))
    try:
        auth.runtime()
        import resource
        import numpy as np
        ...
        if spec['gpu']:
            ...
            c.require(torch.cuda.is_available() and 'A6000' in torch.cuda.get_device_name(0),'Exact GPU class; no substitution')
            ...
            resource_extra={'image_encodings':backend.encodings,'peak_torch_allocated_bytes':torch.cuda.max_memory_allocated(),
