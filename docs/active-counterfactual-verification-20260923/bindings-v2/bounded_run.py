"""New completion budget, independent of accepted preparation receipts."""
import common
import ctypes
...
if __name__ == '__main__':
    ...
    prior = common.BASE / 'bindings-v1/ATTEMPTS.jsonl'
    entries = [common.json.loads(line) for line in prior.read_text().splitlines()]
    ...
    remaining = 14400 - sum(e.get('wall_seconds', 0) for e in entries)
    common.require(remaining > 0, 'Four-hour cumulative budget exhausted')
    def append(value):
    ...
    result.update(label=label, state='finished', wall_seconds=time.monotonic()-start,
                  ...
                  output_bytes=sum(p.stat().st_size for folder in (root,prior.parent) for p in folder.rglob('*') if p.is_file()), gpu_allocations=0)
    if result['output_bytes'] > 1_000_000_000: result['returncode'] = 126
    append(result)
