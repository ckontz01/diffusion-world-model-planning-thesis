...
$acvDestination = 'D:/THESIS-BACKUPS/active-counterfactual-verification-20260923/bindings-v1'
...
if ($acvTarget -ne 'D:\THESIS-BACKUPS\active-counterfactual-verification-20260923\bindings-v1') { throw 'Unexpected destination' }
...
$acvReceipt = [ordered]@{status='VERIFIED';package_commit=$acvHead;remote_head_verified=$true;branch=$acvBranch;
    ...
    note='Only new bindings-v1 preparation outputs. Every member and receipt read back. Existing preparation-v1 and all historical archives untouched.'}
