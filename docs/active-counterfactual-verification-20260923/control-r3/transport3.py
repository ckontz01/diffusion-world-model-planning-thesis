...
HERE=r.REPO/'docs/active-counterfactual-verification-publication-20260923/continuation-r3'
REL='docs/active-counterfactual-verification-20260923/control-r3'
...
exec(compile(request['code'],'<acv0-r3-operation>','exec'),{'CONFIG':request['config'],'json':json,'__name__':'__main__'})
"""
...
def envelope(code,payload=b'',config=None):
    compile(code,'<acv0-r3-operation>','exec')
...
def local():
    ...
    for n,h in r.read(r.ROOT/'SOURCE-MANIFEST.json')['files'].items():r.require(r.sha(r.ROOT/n)==h,'Frozen R3 source: '+n)
    verify_archive(r.ROOT/'source-export.tar',cfg['transport'])
    a=r.read(HERE/'EXECUTION-APPROVAL.json')
    r.require(a=={'schema':'ACV0-control-only-r3','authorized':True,'instruction':instruction(),'binding':b},'Exact additional authority')
...
assert not Path(b['control_source']).exists() and not Path(b['control']).exists(),'Existing R3 namespace; no repeat'
...
command=[pins['python'],'-B','-S',str(root/'controller3.py'),'--approval',str(control/'EXECUTION-APPROVAL.json')]
...
rows=r.lines(control/'CAMPAIGN-R3.jsonl') if (control/'CAMPAIGN-R3.jsonl').exists() else []
...
records={n:r.read(control/n) for n in ('STOP-RESOLUTION.json','MODEL-FREEZE-R3.json','STOP-R3.json') if (control/n).exists()}
technical={n:r.read(run/n/'TECHNICAL.json') for n in ('collect-fit-505',) if (run/n/'TECHNICAL.json').exists()}
