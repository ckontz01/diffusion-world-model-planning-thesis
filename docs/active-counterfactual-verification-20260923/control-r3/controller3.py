"""Exclusive R3 controller: reuse24 completed tasks; only315 new submissions."""
import r3_core as r
import argparse
import os
...
def permitted(spec,jobs):
    r.require(spec['key'] not in {j['key'] for j in jobs[:24]} and spec in jobs[24:],'Existing24/generic restart blocked')
...
def observation(text,job,spec):
    ...
    if fields[1]:r.require(fields[1]=='acv0r3-'+spec['key'],'Wrong scheduler task identity')
...
class Slurm:
    ...
    def submit(self,j):
        ...
        args=['sbatch','--parsable','--no-requeue','--account=superworld','--cpus-per-task=4',
              ...
              '--job-name=acv0r3-'+j['key'],'--output='+str(ctx.run/(j['key']+'-%j.out')),
...
def execute(jobs,existing,scheduler,record,check,gate,budget_check):
    r.require(len(jobs)==339 and len(existing)==24 and [v['spec'] for v in existing]==jobs[:24]
              and existing[0]['job']=='304193' and existing[-1]['job']=='304220'
              and all(v['state']=='COMPLETED' and v['exit_code']=='0:0' for v in existing),'Exact24 reused suppliers')
    r.require(len({j['key'] for j in jobs})==339 and jobs[24]['key']=='collect-fit-505','Exact task order')
    completed=list(existing);gpu=23+sum(v['seconds']*v['gpus'] for v in existing);cpu=0
    r.require(gpu==3015 and len({v['job'] for v in existing})==24,'Prior charges/identities once')
    known={'304189'}|{v['job'] for v in existing};keys={v['spec']['key'] for v in existing}
    for i in range(24,len(jobs)):
        ...
        record({'event':'claim','spec':j,'attempt':1,'reserved_seconds':j['seconds'],
                'campaign_gpu_seconds':gpu,'remaining_gpu_reservation':remaining,'r3_replacements_authorized':0})
    ...
    return {'jobs':completed,'campaign_gpu_seconds':gpu,'campaign_cpu_stage_seconds':cpu,'new_r3_allocations':315,
...
def gates(ctx,name,rows):
    if name=='models':
        ...
        r.write(ctx.control/'MODEL-FREEZE-R3.json',{'unix':time.time(),'original_model_freeze_sha256':r.sha(ctx.run/'MODEL-FREEZE.json')})
    else:
...
def main():
    ...
    def record(row):
        row['unix']=time.time();r.append(ctx.control/'CAMPAIGN-R3.jsonl',row)
        if row['event'] in ('accepted_existing','accepted_new'):r.append(ctx.control/'SCIENTIFIC-TASKS-R3.jsonl',row)
    try:
        ...
        r.write(ctx.control/'STOP-RESOLUTION.json',{'resolved_stop_path':str(ctx.run/'STOP.json'),'resolved_stop_sha256':r.STOP_SHA,
                ...
                'instruction_sha256':ctx.binding['instruction_sha256'],'r3_approval_sha256':ctx.approval_sha,'accepted_existing_sha256':r.sha(ctx.control/'ACCEPTED-EXISTING.json'),
                'scope':'Only documented historical R1 association and R2 status-parser faults; both STOPs retained, no new fault waived','unix':time.time()})
        ...
        r.write(ctx.run/'COMPUTE-COMPLETE.json',{'r3_control':str(ctx.control),'completion_sha256':r.sha(ctx.control/'COMPUTE-COMPLETE.json'),
                'combined_campaign_sha256':r.sha(ctx.control/'COMBINED-CAMPAIGN.json'),'successful_unique_tasks':339,'campaign_allocations':340})
    except BaseException as e:
        r.write(ctx.control/'STOP-R3.json',{'error':repr(e)[:4096],'unix':time.time(),'automatic_retry':False,'action':'Preserve; reconcile; no further continuation authorized'})
        raise
