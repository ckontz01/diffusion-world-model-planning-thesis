"""R3 finalization combines immutable R1/R2 segments and new R3 evidence."""
import r3_core as r
import argparse
import os
...
def accept(ctx,result):
    ...
    r.require(len(accepted)==24 and len(receipt)==24,'All24 dated reuse receipts')
    ...
    r.require(rows[:24]==existing,'Existing24 suppliers exactly once, not new allocations')
    ids=[v['job'] for v in rows];r.require(len(set(ids))==339 and '304189' not in ids,'Successful unique allocation IDs')
    for name,value in [('new_r3_allocations',315),('campaign_allocations',340),('successful_unique_tasks',339),('historical_failed_allocations',1),
    ...
    ledger=r.lines(ctx.control/'CAMPAIGN-R3.jsonl');new_submitted=[v for v in ledger if v['event']=='submitted']
    terminal=[v for v in ledger if v['event']=='terminal'];reused=[v for v in ledger if v['event']=='accepted_existing']
    r.require(len(reused)==24 and [v['job'] for v in reused]==ids[:24] and all(not v['new_submission'] for v in reused),'Exactly24 reused completion events')
    r.require([v['job'] for v in new_submitted]==ids[24:] and terminal==rows[24:] and len(new_submitted)==315,'Only315 new allocation events')
    r.require([v['spec'] for v in new_submitted]==jobs[24:],'Submitted original specifications')
    r.require(not any('unresolved' in v['event'] for v in ledger),'Unresolved R3 attempt')
    ...
    r.require(len(unresolved)==1 and unresolved[0]['event']=='observation_unresolved' and unresolved[0]['job']=='304220'
              and accepted[-1]['job']=='304220' and accepted[-1]['originally_accepted_by_r2_controller'] is False,'Only reconciled R2 status fault resolved')
    scientific=r.lines(ctx.control/'SCIENTIFIC-TASKS-R3.jsonl')
    ...
    frozen=r.read(ctx.control/'MODEL-FREEZE-R3.json')
    r.require(frozen['original_model_freeze_sha256']==r.sha(ctx.run/'MODEL-FREEZE.json') and rows[81]['unix']<=frozen['unix']<=new_submitted[82-24]['unix'],'Both model seals before final-source submission')
    ...
    return {'allocations':[expected_failed]+rows,'successful_task_keys':[j['key'] for j in jobs],
            ...
            'gpu_seconds':gpu,'cpu_stage_seconds':cpu,'historical_r2_allocations':23,'new_r3_allocations':315,'new_r3_replacements':0,'automatic_retries':0,
            'original_r1_segments_unchanged':originals,'r2_segment_sha256':r.sha(Path(ctx.baseline['paths']['r2_control'])/'CAMPAIGN-R2.jsonl'),
            'r3_segment_sha256':r.sha(ctx.control/'CAMPAIGN-R3.jsonl'),'existing24_reused_once':True,'r2_missing304220_acceptance_resolved_in_r3':True,
            ...
            'r3_control_manifest':ctx.binding['control_manifest'],'r3_authorization_sha256':ctx.approval_sha}
...
def inventory(ctx):
    ...
    roots=[('scientific-source',Path(ctx.baseline['paths']['source'])),('r1-control',Path(ctx.baseline['paths']['control'])),
           ...
           ('r3-control-source',r.ROOT),('r3-control-records',ctx.control)]
    ...
    for name in ctx.baseline['inventory']:
        label,rel=name.split('/',1);mapped={'source':'scientific-source','control':'r1-control','run':'run','r2_source':'r2-control-source','r2_control':'r2-control-records'}[label]+'/'+rel
        r.require(mapped in names,'Original member absent from final inventory: '+name)
    r.require(all(n in names for n in ('run/STOP.json','r2-control-records/STOP-R2.json','r2-control-records/STOP-RESOLUTION.json','r3-control-records/STOP-RESOLUTION.json')),'Both historical STOPs and all resolutions archived')
    return items
...
def archive(ctx):
    ...
    r.require(pointer['r3_control']==str(ctx.control) and pointer['completion_sha256']==r.sha(ctx.control/'COMPUTE-COMPLETE.json')
              and pointer['combined_campaign_sha256']==r.sha(ctx.control/'COMBINED-CAMPAIGN.json')
              and pointer['successful_unique_tasks']==339 and pointer['campaign_allocations']==340,'Original-run completion points to authenticated R3 records')
    r.storage(ctx)
    ...
    try:
        ...
        r.write(output/'BACKUP-REQUEST.json',{'archive':dest.as_posix(),'bytes':dest.stat().st_size,'sha256':r.sha(dest),'members':members,
                'package_sha256':r.SCIENCE_MANIFEST,'approval_sha256':r.WORKER_APPROVAL,'r3_control_manifest':ctx.binding['control_manifest'],
                'r3_approval_sha256':ctx.approval_sha,'run_name':ctx.run.name,'archive_wall_seconds':time.monotonic()-started,
