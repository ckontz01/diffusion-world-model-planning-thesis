...
if __name__ == '__main__':
    ...
    publication = common.REPO / 'docs/active-counterfactual-verification-publication-20260923/continuation-r2'
    ...
    remaining = 7200 - sum(e.get('wall_seconds', 0) for e in entries)
    common.require(remaining > 0, 'R2 two-hour cumulative budget exhausted')
    def append(value):
    ...
    result.update(label=label, state='finished', wall_seconds=time.monotonic()-start,
                  ...
                  output_bytes=sum(p.stat().st_size for folder in (root,publication) for p in folder.rglob('*') if p.is_file()), gpu_allocations=0)
