"""ACV0 R2 control-only capability and technical acceptance. Stdlib only."""
import hashlib
import importlib
import json
import os
...
def binding(manifest):
    ...
    require(contract['instruction_sha256']==sha(ROOT/'INSTRUCTION.txt') and contract['baseline_sha256']==sha(ROOT/'BASELINE.json'),'R2 contract bytes')
    name='active-counterfactual-verification-control-r2-'+manifest[:16]
...
class Context:
    def __init__(self,approval):
        a=read(approval)
        require(set(a)=={'schema','authorized','instruction','binding'} and a['schema']=='ACV0-control-only-r2','R2 approval schema')
        if a['authorized'] is not True:raise PermissionError('R2 continuation disabled')
        require(a['binding']==binding(sha(ROOT/'SOURCE-MANIFEST.json')),'Exact R2 continuation binding')
        require(hashlib.sha256(a['instruction'].encode()).hexdigest()==a['binding']['instruction_sha256'],'Exact R2 instruction')
        for name,h in read(ROOT/'SOURCE-MANIFEST.json')['files'].items():
            p=ROOT/name;require(p.parent==ROOT and not p.is_symlink() and sha(p)==h,'R2 control source: '+name)
        ...
        require(self.jobs[0]['key']=='collect-fit-490' and self.jobs[1]['key']=='collect-fit-545','Fixed continuation order')
...
def verify_baseline(ctx,exact_run=False):
    ...
    for label in ('source','control','run'):
...
def stop_guard(ctx):
    ...
    for p in ctx.control.rglob('*'):
        ...
        require(p==ctx.control/'STOP-RESOLUTION.json','Unresolved R2 fault')
        resolved=read(p)
        require(resolved['resolved_stop_path']==str(ctx.run/'STOP.json') and resolved['resolved_stop_sha256']==STOP_SHA
                and resolved['instruction_sha256']==ctx.binding['instruction_sha256'] and resolved['r2_approval_sha256']==ctx.approval_sha
                and resolved['accepted_existing_sha256']==sha(ctx.control/'ACCEPTED-EXISTING.json'),'Exact authorized stop resolution')

def parse_row(row):
    r=row.rstrip('|').split('|') if isinstance(row,str) else list(row)
...
def allocation_contract(spec,row):
    ...
    require(row['job_name']==('acv0r1-' if row['job']=='304193' else 'acv0r2-')+spec['key'],'Exact allocation task name')
...
def accept_existing(ctx):
    p=ctx.run/'collect-fit-490'
    for name,h in EXISTING_HASHES.items():require(sha(p/name)==h,'Pinned existing supplier: '+name)
    row=parse_row(ctx.baseline['scheduler_rows'][1])
    require(row['job']=='304193' and row['seconds']==80,'Existing80-second supplier')
    row.update(spec=ctx.jobs[0],attempt=2,event='existing_completed_supplier',gpu_seconds=103,cpu_seconds=0)
    receipt=verify_worker(ctx,ctx.jobs[0],row)
    receipt.update(event='accepted_existing',accepted_unix=time.time(),message='Accepted existing completed task304193 after the documented hostname-association correction.',
                   originally_accepted_by_r1_controller=False,new_submission=False)
    return row,receipt

def validate_prior_scheduler(ctx,accounting,queue):
    found=[s.rstrip('|').split('|') for s in accounting.splitlines() if 'acv0' in s.lower() or s.split('|')[0] in ('304189','304193')]
    expected=[parse_row(r)['raw_scheduler'] for r in ctx.baseline['scheduler_rows']]
    require(found==expected,'Unknown or changed prior ACV0 attempt')
    require(not any('acv0' in s.lower() or s.split('|')[0] in ('304189','304193') for s in queue.splitlines()),'Live prior campaign job')

def ensure_unstarted(ctx):
    require(not any((ctx.control/n).exists() for n in ('CONTROLLER-STARTED.json','CAMPAIGN-R2.jsonl','ACCEPTED-EXISTING.json','COMPUTE-COMPLETE.json','STOP-R2.json')),'Already-started R2; no restart')
    require(not any((ctx.run/j['key']).exists() for j in ctx.jobs[1:]),'Never-submitted task directory already exists')
    require(not any((ctx.run/n).exists() for n in ('TECHNICAL-TRANCHE-PASSED.json','MODEL-FREEZE.json','PRE-ANALYSIS-ACCOUNTING.json','COMPUTE-COMPLETE.json','final-preservation')),'Unexpected continuation outputs')

def reconcile(ctx):
    verify_baseline(ctx,exact_run=True);stop_guard(ctx);ensure_unstarted(ctx)
    old=ctx.baseline['original_controller'];require(old['pid']==684587 and str(old['start_ticks'])=='893012584','Pinned original process')
    stat=Path('/proc',str(old['pid']),'stat')
    if stat.exists():
        f=stat.read_text().rsplit(')',1)[1].split();require(f[19]!=str(old['start_ticks']) or f[0]=='Z','R1 controller active')
    ...
    return {'unix':time.time(),'failed_job':'304189','completed_supplier':'304193','gpu_seconds':103,
...
def storage(ctx):
    ...
    extra=total(ROOT)+total(ctx.control)
    require(base['source_control_models_analysis']+extra<=500000000,'R2-inclusive source/model/control/analysis cap')
    require(base['full_live_reservation']<=2000000000 and base['inclusive_reservation']<=8000000000,'Full future storage reservation')
    return dict(base,r2_source_and_control_bytes=extra,source_control_models_analysis_including_r2=base['source_control_models_analysis']+extra)
