"""R4 tests share the existing R2 two-hour cumulative preparation ceiling."""
import archive6 as common
import ctypes
...
if __name__ == '__main__':
    ...
    publication = common.REPO / 'docs/active-counterfactual-verification-publication-20260923/preservation-r6'
    ...
    prior=sum((common.lines(root.parent/folder/'ATTEMPTS.jsonl') for folder in ('control-r2','control-r3','control-r4','finalization-r5')),[])
    remaining = 7200 - sum(e.get('wall_seconds', 0) for e in entries+prior)
    common.require(remaining > 0, 'R2+R3+R4 two-hour cumulative budget exhausted')
    def append(value):
    ...
    result.update(label=label, state='finished', wall_seconds=time.monotonic()-start,
                  ...
                  output_bytes=sum(p.stat().st_size for folder in (root,publication,root.parent/'control-r2',publication.parent/'continuation-r2',root.parent/'control-r3',publication.parent/'continuation-r3',root.parent/'control-r4',publication.parent/'continuation-r4',root.parent/'finalization-r5',publication.parent/'finalization-r5') for p in folder.rglob('*') if p.is_file()), gpu_allocations=0)
