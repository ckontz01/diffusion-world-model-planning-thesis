...
HERE=r.REPO/'docs/active-counterfactual-verification-publication-20260923/continuation-r4'
REL='docs/active-counterfactual-verification-20260923/control-r4'
...
exec(compile(request['code'],'<acv0-r4-operation>','exec'),{'CONFIG':request['config'],'json':json,'__name__':'__main__'})
"""
...
def envelope(code,payload=b'',config=None):
    compile(code,'<acv0-r4-operation>','exec')
...
def local():
    ...
    for n,h in r.read(r.ROOT/'SOURCE-MANIFEST.json')['files'].items():r.require(r.sha(r.ROOT/n)==h,'Frozen R4 source: '+n)
    verify_archive(r.ROOT/'source-export.tar',cfg['transport'])
    a=r.read(HERE/'EXECUTION-APPROVAL.json')
    r.require(a=={'schema':'ACV0-control-only-r4','authorized':True,'instruction':instruction(),'binding':b},'Exact additional authority')
...
assert not Path(b['control_source']).exists() and not Path(b['control']).exists(),'Existing R4 namespace; no repeat'
for name,info in base['inventory'].items():
 label,rel=name.split('/',1);p=Path(base['paths'][label])/rel
 h=hashlib.sha256()
 with p.open('rb') as stream:
  for chunk in iter(lambda:stream.read(1048576),b''):h.update(chunk)
 assert p.stat().st_size==info['bytes'] and h.hexdigest()==info['sha256'],'Original bytes changed: '+name
...
for n,raw in [('EXECUTION-APPROVAL.json',approval),('AUTHORIZATION-PROVENANCE.json',json.dumps(package['provenance'],sort_keys=True).encode()),
              ('SMALL-PACKAGE-DELIVERY.json',json.dumps(package['delivery'],sort_keys=True).encode()),
...
command=[pins['python'],'-B','-S',str(root/'controller4.py'),'--approval',str(control/'EXECUTION-APPROVAL.json')]
...
rows=r.lines(control/'CAMPAIGN-R4.jsonl') if (control/'CAMPAIGN-R4.jsonl').exists() else []
...
if ids:
 ...
 assert p.returncode==0,p.stderr;accounting=[s for s in p.stdout.splitlines() if s.split('|')[0] in ids]
records={n:r.read(control/n) for n in ('STOP-RESOLUTION.json','MODEL-FREEZE-R4.json','STOP-R4.json') if (control/n).exists()}
technical={n:r.read(run/n/'TECHNICAL.json') for n in ('collect-fit-256',) if (run/n/'TECHNICAL.json').exists()}
