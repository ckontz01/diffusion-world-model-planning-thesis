"""ACV0 R4 status-only continuation; original workers and approvals unchanged."""
import hashlib
import importlib
import json
import os
...
def binding(manifest):
    ...
    require(contract['instruction_sha256']==sha(ROOT/'INSTRUCTION.txt') and contract['baseline_sha256']==sha(ROOT/'BASELINE.json'),'R4 contract bytes')
    name='active-counterfactual-verification-control-r4-'+manifest[:16]
...
class Context:
    def __init__(self,approval):
        a=read(approval)
        require(set(a)=={'schema','authorized','instruction','binding'} and a['schema']=='ACV0-control-only-r4','R4 approval schema')
        if a['authorized'] is not True:raise PermissionError('R4 continuation disabled')
        require(a['binding']==binding(sha(ROOT/'SOURCE-MANIFEST.json')),'Exact R4 continuation binding')
        require(hashlib.sha256(a['instruction'].encode()).hexdigest()==a['binding']['instruction_sha256'],'Exact R4 instruction')
        for name,h in read(ROOT/'SOURCE-MANIFEST.json')['files'].items():
            p=ROOT/name;require(p.parent==ROOT and not p.is_symlink() and sha(p)==h,'R4 control source: '+name)
        ...
        require(self.jobs[0]['key']=='collect-fit-490' and self.jobs[37]['key']=='collect-fit-256','Fixed continuation order')
        require([v['spec'] for v in self.baseline['successful_rows']]==self.jobs[:37],'All37 unchanged suppliers')
...
def stop_guard(ctx):
    ...
    require(sha(Path(ctx.baseline['paths']['r3_control'])/'STOP-R3.json')==ctx.binding['resolved_r3_stop_sha256'],'Only exact resolved R3 STOP permitted')
    ...
    for p in ctx.control.rglob('*'):
        ...
        require(p==ctx.control/'STOP-RESOLUTION.json','Unresolved R4 fault')
        resolved=read(p)
        require(resolved['resolved_stop_path']==str(ctx.run/'STOP.json') and resolved['resolved_stop_sha256']==STOP_SHA
                ...
                and resolved['resolved_r3_stop_sha256']==ctx.binding['resolved_r3_stop_sha256']
                and resolved['instruction_sha256']==ctx.binding['instruction_sha256'] and resolved['r4_approval_sha256']==ctx.approval_sha
...
def allocation_contract(spec,row):
    ...
    require(row['job_name']==historic.get(row['job'],'acv0r4-'+spec['key']),'Exact allocation task name')
...
def accept_existing(ctx):
    rows=ctx.baseline['successful_rows'];require(len(rows)==37 and [v['spec'] for v in rows]==ctx.jobs[:37],'Exact37 reused rows')
    require(rows[0]['job']=='304193' and rows[23]['job']=='304220' and rows[-1]['job']=='304237' and rows[-1]['seconds']==157,'Pinned boundary suppliers')
    accepted=[]
    for spec,row in zip(ctx.jobs[:37],rows):
        ...
        receipt.update(event='accepted_existing',accepted_unix=time.time(),new_submission=False,
            originally_accepted_by_r2_controller=row['job'] in {v['job'] for v in ctx.baseline['r2_suppliers']},
            originally_accepted_by_r3_controller=row['job'] in {v['job'] for v in ctx.baseline['r3_suppliers']},
            message='Reused completion with current R4 authentication, never recomputed. R3 resolved old304220 acceptance; missing R3 acceptance of304237 resolved now, not backdated.')
        accepted.append(receipt)
    require(23+sum(v['seconds']*v['gpus'] for v in rows)==4862,'All38 prior allocation charges')
...
def validate_prior_scheduler(ctx,accounting,queue):
    ...
    require(len(found)==38 and {v['job']:v for v in found}==expected,'Unknown or changed prior ACV0 attempt')
...
def ensure_unstarted(ctx):
    require(not any((ctx.control/n).exists() for n in ('CONTROLLER-STARTED.json','CAMPAIGN-R4.jsonl','ACCEPTED-EXISTING.json','COMPUTE-COMPLETE.json','STOP-R4.json')),'Already-started R4; no restart')
    require(not any((ctx.run/j['key']).exists() for j in ctx.jobs[37:]),'Never-submitted task directory already exists')
...
def reconcile(ctx):
    ...
    for old in ({'pid':684587,'start_ticks':'893012584'},ctx.baseline['r2_controller'],ctx.baseline['r3_controller']):
    ...
    return {'unix':time.time(),'failed_job':'304189','completed_suppliers':[v['job'] for v in ctx.baseline['successful_rows']],'gpu_seconds':4862,
...
def storage(ctx):
    ...
    extra=total(ROOT)+total(ctx.control)+sum(total(ctx.baseline['paths'][name]) for name in ('r2_source','r2_control','r3_source','r3_control'))
    require(base['source_control_models_analysis']+extra<=500000000,'R2/R4-inclusive source/model/control/analysis cap')
    require(base['full_live_reservation']<=2000000000 and base['inclusive_reservation']<=8000000000,'Full future storage reservation')
    return dict(base,r2_r4_source_and_control_bytes=extra,source_control_models_analysis_including_r2_r4=base['source_control_models_analysis']+extra)
