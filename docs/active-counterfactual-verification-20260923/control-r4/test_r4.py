"""Stdlib-only synthetic R4 regressions. No research model/data/physics calls."""
import ast,copy,importlib.util,os,subprocess,sys,tempfile,time,unittest
from pathlib import Path
from types import SimpleNamespace as NS
from unittest.mock import Mock,patch
import r4_core as r
import controller4 as ctl
import finalize4 as final
...
class Fixture(OriginalFixture):
    def __init__(self,path):
        ...
        self.r3_source=self.root/'r3-source';self.r3_source.mkdir();r.write(self.r3_source/'control.json',{'artificial':True})
        self.r3_control=self.root/'old-r3';self.r3_control.mkdir()
        r.write(self.r3_control/'STOP-R3.json',ACTUAL['r3_stop'])
        r.write(self.r3_control/'STOP-RESOLUTION.json',{'r1_r2_resolved':True})
        self.binding.update(resolved_r3_stop_sha256=r.sha(self.r3_control/'STOP-R3.json'),r3_source_manifest='synthetic-r3')
        for row in ACTUAL['r3_ledger']:r.append(self.r3_control/'CAMPAIGN-R3.jsonl',row)
        for row in ACTUAL['r3_scheduler_observations']:r.append(self.r3_control/'SCHEDULER-OBSERVATIONS.jsonl',row)
        ...
        self.baseline['paths']={'source':str(self.science_root),'control':str(self.r1_control),'run':str(self.run),
                                ...
                                'r3_source':str(self.r3_source),'r3_control':str(self.r3_control)}
    ...
    def row(self,j,job,existing=False):
        ...
        row['job_name']=r.read(r.ROOT/'CONTRACT.json')['historical_job_names'].get(job,'acv0r4-'+j['key'])
    ...
    def resolution(self,accepted):
        ...
        r.write(self.control/'STOP-RESOLUTION.json',{'resolved_stop_path':str(self.run/'STOP.json'),'resolved_stop_sha256':self.stop_sha,
            ...
            'resolved_r3_stop_sha256':self.binding['resolved_r3_stop_sha256'],
            'r4_approval_sha256':self.approval_sha,'accepted_existing_sha256':r.sha(self.control/'ACCEPTED-EXISTING.json')})
    def run_all(self):
        existing,accepted=r.accept_existing(self);self.resolution(accepted);calls=[];gates=[]
        def record(row):
            row['unix']=time.time();r.append(self.control/'CAMPAIGN-R4.jsonl',row)
            if row['event'] in ('accepted_existing','accepted_new'):r.append(self.control/'SCIENTIFIC-TASKS-R4.jsonl',row)
...
class R4Tests(unittest.TestCase):
    def test_blank_final_field_and_optional_delimiter(self):
        ...
        for v in (blank,raw.replace('|157|','||'),raw.replace('|157|','|unknown|')):
    ...
    def test_state_before_nonterminal_numeric_fields(self):
        ...
        for state in sorted(ctl.ACTIVE):
            raw='800001|acv0r4-'+j['key']+'|'+state+'|||||||||'
            row,reason=ctl.observation(raw,'800001',j)
            self.assertIsNone(row);self.assertEqual(reason,'active:'+state)
        for raw in ('','800001|','800001|acv0r4-'+j['key']+'|COMPLETED|0:0'):
            self.assertIsNone(ctl.observation(raw,'800001',j)[0])
        for raw in ('999|x|RUNNING','800001|wrong|RUNNING','800001|acv0r4-'+j['key']+'|REQUEUED',
    ...
    def test_status_only_wait_delayed_terminal_and_single_submission(self):
        with tempfile.TemporaryDirectory() as tmp:
            x=Fixture(tmp);s=ctl.Slurm(x);j=x.jobs[37];good='|'.join(x.raw(x.row(j,'800001')))
            partial=good.rsplit('|',1)[0]+'|';responses=['800001','', '800001|acv0r4-'+j['key']+'|RUNNING|||||||||',partial,good]
            calls=[]
        ...
        for stdout,rc in [('',0),('999|wrong|RUNNING',0),('',1)]:
            with tempfile.TemporaryDirectory() as tmp:
                ...
                with patch.object(r,'stop_guard'),patch.object(subprocess,'run',calls),patch.object(time,'sleep'):
                    with self.assertRaises(ValueError):ctl.Slurm(x).wait('800001',c.grid()[37])
    ...
    def test_all37_reused_readonly_no_scheduler_model_or_physics(self):
        with tempfile.TemporaryDirectory() as tmp:
            x=Fixture(tmp)
            ...
            self.assertEqual(len(rows),37);self.assertEqual(rows[-1]['job'],'304237')
            ...
            self.assertFalse(receipts[-1]['originally_accepted_by_r3_controller'])
            self.assertTrue(receipts[23]['originally_accepted_by_r3_controller'])
    ...
    def test_corrupt_existing_and_all37_submission_paths_blocked(self):
        with tempfile.TemporaryDirectory() as tmp:
    ...
    def test_complete302_dispatch_finalization_and_archive(self):
        with tempfile.TemporaryDirectory() as tmp:
            x=Fixture(tmp)
            with patch.object(r,'STOP_SHA',x.stop_sha):
                result,calls,gates=x.run_all()
                self.assertEqual(len(calls),302);self.assertEqual(calls[0]['key'],'collect-fit-256')
                self.assertFalse(set(j['key'] for j in calls)&set(j['key'] for j in x.jobs[:37]))
                self.assertEqual(gates,[('models',45),('analysis',301)])
                self.assertEqual(result['campaign_gpu_seconds'],159062);self.assertEqual(result['campaign_cpu_stage_seconds'],21600)
                claims=[v for v in r.lines(x.control/'CAMPAIGN-R4.jsonl') if v['event']=='claim']
                self.assertEqual(claims[0]['campaign_gpu_seconds'],4862)
                ...
                for name in ('run/STOP.json','r2-control-records/STOP-R2.json','r2-control-records/STOP-RESOLUTION.json',
                             'r3-control-records/STOP-R3.json','r3-control-records/SCHEDULER-OBSERVATIONS.jsonl',
                             'r4-control-records/STOP-RESOLUTION.json','run/provenance-failed-v2/FAILURE.json','run/collect-fit-1342/SEAL.json'):
                ...
                r.write(x.run/'COMPUTE-COMPLETE.json',{'r4_control':str(x.control),'completion_sha256':r.sha(x.control/'COMPUTE-COMPLETE.json'),
                ...
                r.write(x.control/'STOP-R4.json',{'error':'new fault'})
    ...
    def test_exact_stops_retained_new_changed_or_unresolved_block(self):
        for change in ('r1','r2','r3','new-run','new-control'):
            with tempfile.TemporaryDirectory() as tmp:
                x=Fixture(tmp)
                with patch.object(r,'STOP_SHA',x.stop_sha):
                    _,a=r.accept_existing(x);x.resolution(a);r.stop_guard(x)
                    if change in ('r1','r2','r3'):
                        path={'r1':x.run/'STOP.json','r2':x.r2_control/'STOP-R2.json','r3':x.r3_control/'STOP-R3.json'}[change]
    ...
    def test_new_failure_charged_and_never_retried(self):
        for mode in ('failed','ambiguous'):
            ...
            if mode=='failed':self.assertEqual(events[-1]['campaign_gpu_seconds'],4869)
    ...
    def test_bad_existing_rows_and_gate_rejection_stop_dispatch(self):
        ...
        self.assertEqual(len(calls),45);self.assertEqual(calls[-1]['key'],'fit-ordinary')

    def test_final_scheduler_exact340_and_no_live_unknown(self):
        with tempfile.TemporaryDirectory() as tmp:
            x=Fixture(tmp)
            rows=ACTUAL['successful_rows']+[x.row(j,str(800000+i)) for i,j in enumerate(x.jobs[37:])]
    ...
    def test_original_sources_storage_and_python39(self):
        for folder in ('bindings-r1','control-r2','control-r3'):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.object(dispatch,'storage',return_value=base):
                got=r.storage(x);self.assertEqual(got['r2_r4_source_and_control_bytes'],r.total(r.ROOT)+r.total(x.control)+r.total(x.r2_source)+r.total(x.r2_control)+r.total(x.r3_source)+r.total(x.r3_control))
    ...
    def test_disabled_authority_and_binary_transport(self):
        with tempfile.TemporaryDirectory() as tmp:
            path=Path(tmp)/'false.json';r.write(path,{'schema':'ACV0-control-only-r4','authorized':False,'instruction':'','binding':{}})
            for args in (['controller4.py','--approval',str(path)],['finalize4.py','archive','--approval',str(path)]):
                p=subprocess.run([sys.executable,'-B','-S',str(r.ROOT/args[0])]+args[1:],capture_output=True)
                self.assertNotEqual(p.returncode,0);self.assertIn(b'R4 continuation disabled',p.stderr)
        ...
        self.assertEqual(t.instruction(),'okay do scheduled checks every 2 hours, and if any TECHNICAL issues arise, I give you full permission to resolve them and resume the jobs. (do not of me to approve)')

    def test_exact_observed_placeholder_and_no_broader_identity_waiver(self):
        j=c.grid()[36]
        raw=next(v['stdout'] for v in ACTUAL['r3_scheduler_observations'] if v['job']=='304237')
        row,reason=ctl.observation(raw,'304237',j)
        self.assertIsNone(row);self.assertEqual(reason,'incomplete-pending-allocation-placeholder')
        # Exact ID is mandatory even for the placeholder; terminal names remain strict.
        for bad in (raw.replace('304237','999999'),raw.replace('PENDING','COMPLETED'),raw.replace('PENDING','RUNNING'),
                    raw.replace('allocation','other'),raw.replace('gpu09','gpu10'),raw.replace('|0|0|','|1|0|'),
                    raw.replace('Unknown','normal-a6000'),raw.replace('superworld','other')):
            with self.assertRaises(ValueError):ctl.observation(bad,'304237',j)
        with self.assertRaises(ValueError):ctl.observation(raw,'304237',c.grid()[80])
        good=ACTUAL['scheduler_raw'][-1].replace('acv0r3-','acv0r4-')
        self.assertEqual(ctl.observation(good,'304237',j)[0]['seconds'],157)

    def test_placeholder_bounded_polling_and_then_strict_terminal(self):
        raw=next(v['stdout'] for v in ACTUAL['r3_scheduler_observations'] if v['job']=='304237')
        good=ACTUAL['scheduler_raw'][-1].replace('acv0r3-','acv0r4-')
        for eventually_complete in (False,True):
            with tempfile.TemporaryDirectory() as tmp:
                responses=[NS(returncode=0,stdout=raw,stderr='')]*8+[NS(returncode=0,stdout=good if eventually_complete else raw,stderr='')]
                calls=Mock(side_effect=responses)
                with patch.object(r,'stop_guard'),patch.object(subprocess,'run',calls),patch.object(time,'sleep') as sleep:
                    scheduler=ctl.Slurm(NS(control=Path(tmp)))
                    if eventually_complete:self.assertEqual(scheduler.wait('304237',c.grid()[36])['state'],'COMPLETED')
                    else:
                        with self.assertRaisesRegex(ValueError,'beyond8'):scheduler.wait('304237',c.grid()[36])
                self.assertEqual(calls.call_count,9);self.assertEqual(sleep.call_count,8)
                self.assertTrue(all(v.args[0][0]=='sacct' for v in calls.call_args_list))
                self.assertEqual(r.lines(Path(tmp)/'SCHEDULER-OBSERVATIONS.jsonl')[0]['stdout'],raw)
