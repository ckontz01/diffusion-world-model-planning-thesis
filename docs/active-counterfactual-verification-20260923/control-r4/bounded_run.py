...
if __name__ == '__main__':
    ...
    publication = common.REPO / 'docs/active-counterfactual-verification-publication-20260923/continuation-r4'
    ...
    prior=common.lines(root.parent/'control-r2/ATTEMPTS.jsonl')+common.lines(root.parent/'control-r3/ATTEMPTS.jsonl')
    ...
    result.update(label=label, state='finished', wall_seconds=time.monotonic()-start,
                  ...
                  output_bytes=sum(p.stat().st_size for folder in (root,publication,root.parent/'control-r2',publication.parent/'continuation-r2',root.parent/'control-r3',publication.parent/'continuation-r3') for p in folder.rglob('*') if p.is_file()), gpu_allocations=0)
