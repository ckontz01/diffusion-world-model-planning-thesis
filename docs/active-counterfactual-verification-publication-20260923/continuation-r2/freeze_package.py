...
REPO=HERE.parents[2]
ROOT=REPO/'docs/active-counterfactual-verification-20260923/control-r2'
...
def main():
    ...
    total=sum(v['wall_seconds'] for v in finished);r.require(total<7200,'Cumulative testing budget')
    ...
    r.write(ROOT/'FREEZE-RECEIPT.json',{'unix':time.time(),'authority':'Exact supplied R2 instruction under standing delegation; not fresh direct user signature',
    ...
    r.write(ROOT/'SOURCE-MANIFEST.json',{'schema':'ACV0-control-only-r2-source','files':files,'original_scientific_manifest':r.SCIENCE_MANIFEST})
    manifest=r.sha(ROOT/'SOURCE-MANIFEST.json');binding=r.binding(manifest)
    template={'schema':'ACV0-control-only-r2','authorized':False,'instruction':'','binding':binding}
    ...
    r.write(HERE/'AUTHORIZATION-PROVENANCE.json',{'basis':'Explicit EXECUTE ACV0 CONTROL-ONLY CONTINUATION R2 supplied by user under standing delegation; not fresh direct user signature',
        'instruction_sha256':binding['instruction_sha256'],'reviewed_fault_commit':binding['reviewed_fault_commit'],'control_manifest':manifest,
        ...
        'scope':'Reuse completed304193; 338 never-submitted original tasks starting545; corrected exact hostname association, separate controller/ledgers/finalizer; zero R2 replacements/retries/scientific changes',
