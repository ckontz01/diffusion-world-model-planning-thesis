...
ATTACHMENT=HERE/'RECOVERY-INSTRUCTION.txt'
ROOT='/lustreFS/data/superworld/ckontzias/thesis'
MANIFEST_SHA=hashlib.sha256((PACKAGE/'SOURCE-MANIFEST.json').read_bytes()).hexdigest()
SOURCE=ROOT+'/snapshots/active-counterfactual-verification-recovery-r1-'+MANIFEST_SHA[:16]
CONTROL=ROOT+'/staging/active-counterfactual-verification-recovery-r1-'+MANIFEST_SHA[:16]
RUN=ROOT+'/experiments/active-counterfactual-verification-pilot-v1/run-'+MANIFEST_SHA[:16]
...
REL='docs/active-counterfactual-verification-20260923/bindings-r1'
TRANSPORT=json.loads((PACKAGE/'SOURCE-TRANSPORT.json').read_text())
ARCHIVE_SHA=TRANSPORT['sha256']
ARCHIVE_BYTES=TRANSPORT['bytes']
MEMBERS=len(TRANSPORT['members'])
...
def local():
    expected={'SOURCE-MANIFEST.json':MANIFEST_SHA,'source-export.tar':ARCHIVE_SHA,
              ...
              'GRID.json':read(PACKAGE/'SOURCE-MANIFEST.json')['files'][REL+'/GRID.json']}
    ...
    assert hashlib.sha256(json.dumps(grid,sort_keys=True,separators=(',',':'),allow_nan=False).encode()).hexdigest()==read(PACKAGE/'RECOVERY-CONTRACT.json')['new_grid_sha256']
...
def envelope(code,payload=b''):
    ...
    config={'source':SOURCE,'control':CONTROL,'run':RUN,'rel':REL,
            ...
            'controller_runtime':read(PACKAGE/'CONTROLLER-RUNTIME.json'),'prior':read(PACKAGE/'PRIOR-ATTEMPT.json')}
...
a=subprocess.run(['sacct','-X','-n','-P','--starttime=2026-09-23','--user='+os.environ['USER'],'--format=JobIDRaw,JobName%100,State,ExitCode,ElapsedRaw,AllocCPUS,AllocTRES,NodeList,Partition,QOS,Account'],capture_output=True,text=True,timeout=30)
assert q.returncode==a.returncode==0,(q.stderr,a.stderr)
prior=[line.split('|') for line in a.stdout.splitlines() if 'acv0' in line.lower() or line.split('|')[0]=='304189']
assert prior==[CONFIG['prior']['scheduler_row']],prior
assert not any('acv0' in line.lower() or line.startswith('304189|') for line in q.stdout.splitlines()),q.stdout
for name,info in CONFIG['prior']['members'].items():
 label,rel=name.split('/',1);path=Path(CONFIG['prior']['paths'][label])/rel
 assert path.stat().st_size==info['bytes'] and hashlib.sha256(path.read_bytes()).hexdigest()==info['sha256'],name
verified=[]
...
sys.path.insert(0,str(source/CONFIG['rel']))
import common as c
import controller_runtime,recovery
...
def main():
    ...
    if a.mode=='preflight':write(HERE/'LOCAL-PREFLIGHT.json',local());once('REMOTE-PREFLIGHT',PREFLIGHT)
        ...
        write(HERE/'AUTHORIZATION-PROVENANCE.json',{'basis':'Explicit EXECUTE ACV0 instruction supplied here under standing user delegation; not a newly obtained direct user signature',
          ...
          'recovery_contract':approval['recovery'],
          'technical_recovery_scope':'Exact-device early gate; sole collect-fit-490 replacement attempt2 at1740/1620 seconds; prior failed304189 and23GPU seconds preserved; no scientific changes or further retries',
          'old_source_manifest':'5630b222e8a88d0eaa45408876929724734d1f992afa657131ac770fb0400fa9',
          'unlaunched_v1_paths_verified_absent':OLD_PATHS,'new_hash_namespace_reason':'Explicit R1 authorization, separate from preserved failed v2; one replacement only',
