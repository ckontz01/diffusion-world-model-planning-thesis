...
def closure():
    paths=set(c.ROOT.glob('*.py'))|set(c.ROOT.glob('*.sh'))|set(c.ROOT.glob('*.md'))
...
def freeze():
    ...
    test=c.read(c.ROOT/'TEST-ACCEPTANCE-v4.json');c.require(test['passed'],'Integration receipt')
    ...
    c.require(sum(r.get('wall_seconds',0) for r in rows)<=14400 and c.bytes_in(c.ROOT)<=1_000_000_000,'Preparation envelope')
    c.write(c.ROOT/'PREPARATION-RECEIPT.json',dict(attempts=rows,wall_seconds=sum(r.get('wall_seconds',0) for r in rows),
                ...
                artifact_bytes=c.bytes_in(c.ROOT),tests_sha256=c.sha(c.ROOT/'TEST-ACCEPTANCE-v4.json')))
...
def backup_package():
    ...
    dest=Path('D:/THESIS-BACKUPS')/c.NAMESPACE/('preparation-runtime-v1-'+h[:16]);dest.mkdir(parents=True,exist_ok=False)
