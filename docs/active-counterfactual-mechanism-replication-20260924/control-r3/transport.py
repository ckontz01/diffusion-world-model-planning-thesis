...
PUB=r.REPO/'docs/active-counterfactual-mechanism-replication-publication-20260924/recovery-r3'
def remote(code):
...
def main():
    ...
    if a.operation=='backup':
        import preserve_r1
        # Complete archive capability stays original; new request includes all R1/R2/R3 roots.
    ...
    if a.operation=='stage':
        ...
        tar=PUB/'SOURCE-PACKAGE.tar';r.require(r.sha(tar)==receipt['archive']['sha256'] and r.sha(receipt['ssd_path'])==receipt['archive']['sha256'],'Verified R3 source SSD bytes')
...
result=dict(unix=time.time(),recovery=ctx.binding['manifest'],approval=ctx.approval_sha,submitted=list(state['submitted'].values()),accepted=len(state['accepted']),carried_success='fit-pair1-joint/304593',accepted_fits=1+sum(e['key'].startswith('fit-') for e in state['accepted'].values()),accepted_evaluations=sum(e['key'].startswith('evaluate-') for e in state['accepted'].values()),gpu_seconds=state['gpu_seconds'],cpu_seconds=state['cpu_seconds']+19,unresolved_claims=sorted(set(state['claimed'])-set(state['submitted'])),unaccepted=sorted(set(state['submitted'])-set(state['accepted'])),prior_failed_attempts=['304589','304591'])
