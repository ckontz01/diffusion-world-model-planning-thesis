...
def reservation(jobs,terminals):
    ...
    c.require(gpu<=c.caps()['gpu_allocation_seconds'] and cpu+19<=36008,'Full future compute reservation including retained19 seconds')
    return dict(gpu_seconds=gpu,cpu_stage_seconds=cpu+19,prior_cpu_stage_seconds=19)
...
def command(spec,approval,run):
    ...
    return cmd+[str(c.ROOT/'run_worker.sh'),str(c.ROOT),str(approval),str(run),spec['key'],str(spec['gpu'])]
...
def campaign(auth,control,sched,*,jobs=None,accept=None,reuse=None,freeze=None,gate=None,storage_check=None):
    ...
    def event(kind,**v):
        payload=dict(event=kind,**v)
        payload.setdefault('unix',time.time())
        c.append(control/'CAMPAIGN.jsonl',payload)
    try:
        if reuse: reuse()
        for spec in jobs:
            ...
            c.write(auth.run/'submissions-r3'/(spec['key']+'.json'),dict(job=job,spec=spec,package=auth.approval['package_sha256']))
        ...
        c.write(control/'COMPUTE-COMPLETE.json',dict(unix=time.time(),tasks=len(completed)+1,carried_tasks=1,episodes=eval_count,
...
if __name__=='__main__':raise SystemExit('Use controller.py with separate recovery authority')
