...
def contract():
    ...
    baseline=r.c.json.loads(r.read(r.ROOT/'BASELINE-TRANSPORT.json')['stdout'])
    caps=dict(r.c.caps());caps['cpu_stage_allocation_seconds']=36008;caps['cpu_core_seconds_reservation']=144032
    r.write(r.ROOT/'CONTRACT.json',dict(schema='ACVM1-R3-control-continuation',instruction_sha256=r.sha(r.ROOT/'INSTRUCTION.txt'),authority='Direct user technical-recovery and launch authorization; one sealed R2 success carried',baseline_sha256=r.sha(r.ROOT/'BASELINE-TRANSPORT.json'),science_manifest=r.SCIENCE,worker_approval=r.APPROVAL,grid=r.c.bindings()['grid'],roles=r.c.bindings()['ROLES.json'],model_reuse=r.c.bindings()['MODEL-REUSE.json'],prior_jobs=['304589','304591','304593'],prior_attempts=3,prior_successful_tasks=1,max_new_attempts=8196,max_total_attempts=8199,successful_logical_tasks=8197,evaluation_episodes=8192,replacement_attempts=0,never_submitted_tasks=8196,prior_gpu_seconds=baseline['prior_gpu_seconds'],prior_cpu_seconds=baseline['prior_cpu_seconds'],caps=caps,local_test_wall_seconds=3600,local_threads=4,local_ram_gib=8,local_bytes=250000000,scientific_changes=False,no_automatic_retry_in_package=True))
...
def freeze():
    ...
    r.write(r.ROOT/'SOURCE-MANIFEST.json',dict(schema='ACVM1-control-r3-source',files=files,scientific_source_unchanged=r.SCIENCE))
    PUB.mkdir(exist_ok=False,parents=True)
    a=dict(schema='ACVM1-recovery-r3',authorized=True,instruction=(r.ROOT/'INSTRUCTION.txt').read_text(encoding='utf8'),binding=r.binding())
...
def backup():
    ...
    dest=Path('D:/THESIS-BACKUPS/active-counterfactual-mechanism-replication-v1')/('recovery-r3-'+b['manifest'][:16]);dest.mkdir(exist_ok=False,parents=True)
