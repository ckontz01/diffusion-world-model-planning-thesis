...
def archive(auth,control,acceptance):
    ...
    try:
        ctx=recovery.Context(control/'EXECUTION-APPROVAL.json');recovery.baseline(ctx);roots=dict(science_source=c.REPO,r1_source=Path(recovery.R1_SOURCE),r2_source=Path(recovery.R2_SOURCE),r3_source=recovery.ROOT,original_control=ctx.old_control,r1_control=ctx.r1_control,r2_control=ctx.r2_control,r3_control=control,run=auth.run)
