...
def reservation(jobs,terminals):
    ...
    c.require(gpu<=c.caps()['gpu_allocation_seconds'] and cpu+8<=36008,'Full future compute reservation including retained8 seconds')
    return dict(gpu_seconds=gpu,cpu_stage_seconds=cpu+8,prior_cpu_stage_seconds=8)
...
def campaign(auth,control,sched,*,jobs=None,accept=None,reuse=None,freeze=None,gate=None,storage_check=None):
    ...
    try:
        ...
        for spec in jobs:
            ...
            c.write(auth.run/'submissions-r2'/(spec['key']+'.json'),dict(job=job,spec=spec,package=auth.approval['package_sha256']))
