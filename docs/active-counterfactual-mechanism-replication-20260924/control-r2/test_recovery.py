...
class Tests(unittest.TestCase):
    def test_authenticated_tree_precedes_colliding_installed_package(self):
        import json,os
        with tempfile.TemporaryDirectory() as tmp:
            root=Path(tmp);base=root/'docs/active-counterfactual-verification-20260923';base.mkdir(parents=True)
            (base/'bindings-r1').mkdir()
            (base/'tree.py').write_text('finite = 17\n')
            (base/'model.py').write_text('from tree import finite\n')
            installed=root/'site';(installed/'tree').mkdir(parents=True)
            (installed/'tree/__init__.py').write_text('finite = 0\n')
            manifest=root/'SOURCE-MANIFEST.json'
            c.write(manifest,dict(files={p.relative_to(root).as_posix():c.sha(p) for p in (base/'tree.py',base/'model.py')}))
            original=list(sys.path);old_tree=sys.modules.pop('tree',None);old_model=sys.modules.pop('model',None)
            try:
                sys.path.insert(0,str(installed))
                with patch.object(c,'REPO',root),patch.object(c,'ROOT',root):
                    r.authenticated_imports()
                    import tree,model
                    self.assertEqual(tree.finite,17)
                    self.assertEqual(model.finite,17)
                    self.assertEqual(Path(tree.__file__).resolve(),(base/'tree.py').resolve())
            finally:
                sys.path[:]=original
                sys.modules.pop('tree',None);sys.modules.pop('model',None)
                if old_tree is not None:sys.modules['tree']=old_tree
                if old_model is not None:sys.modules['model']=old_model

    def test_failed_worker_output_preserved_exclusively(self):
        with tempfile.TemporaryDirectory() as tmp:
            root=Path(tmp);run=root/'run';control=root/'control';control.mkdir();failed=run/'fit-pair1-joint';failed.mkdir(parents=True)
            c.write(failed/'FAILURE.json',dict(error='artificial'))
            identity=c.sha(failed/'FAILURE.json')
            ctx=types.SimpleNamespace(run=run,control=control,baseline={'inventory':{'run/fit-pair1-joint/FAILURE.json':{'sha256':identity}}})
            controller.preserve_failed_output(ctx)
            historic=run/'recovery-history/r1/fit-pair1-joint/FAILURE.json'
            self.assertEqual(c.sha(historic),identity)
            self.assertFalse(failed.exists())
            with self.assertRaises(ValueError):controller.preserve_failed_output(ctx)
    def test_disabled_before_access(self):
        with tempfile.TemporaryDirectory() as tmp:
            p=Path(tmp)/'false.json';c.write(p,dict(schema='ACVM1-recovery-r2',authorized=False,instruction='',binding={}))
    ...
    def test_only_submission_and_recovery_technical_binding_changed(self):
        ...
        self.assertIn("'submissions-r2'",new)
    ...
    def test_command_keeps_resources_and_retains_failed_logs(self):
        ...
        self.assertTrue(str(r.slurm_log(ctx.run,'fit-pair1-joint','.slurm.err')).endswith('.r2.slurm.err'))
        self.assertTrue(str(r.slurm_log(ctx.run,'fit-pair1-joint','.err')).endswith('.r2.err'))
    ...
    def campaign(self,fault=None):
        tmp=tempfile.TemporaryDirectory();self.addCleanup(tmp.cleanup);root=Path(tmp.name);run=root/'run';control=root/'control';run.mkdir();control.mkdir();(run/'submissions-r2').mkdir()
    ...
    def test_actual8197_campaign_and8199_combined_finalization(self):
        auth,control,state,flags=self.campaign();self.assertTrue(all(flags.values()));self.assertEqual(len(state['accepted']),8197)
        self.assertEqual(len(list((auth.run/'submissions-r2').iterdir())),8197)
        ...
        rows=ctx.baseline['failed_rows']+[e['row'] for e in state['terminal'].values()]
        real_read=c.read
        def read(path):
        ...
        with patch.object(a,'worker',side_effect=lambda *args:dict(key=args[1]['key'])),patch('models.check_freeze',return_value={'unix':0}),patch.object(a,'check_gate',return_value={'unix':gate_time}),patch.object(c,'read',side_effect=read),patch.object(r,'read',side_effect=read):
            result=finalize.reconcile(ctx,rows);self.assertEqual(result['attempts'],8199);self.assertEqual(result['successful_tasks'],8197)
            with self.assertRaises(ValueError):finalize.reconcile(ctx,rows[:-1])
            bad=[dict(x,seconds=9) if x['job']=='304591' else x for x in rows]
    ...
    def test_full_resource_caps_and_all_archive_roots(self):
        import storage
        f=storage.footprint(c.grid());self.assertEqual(d.reservation(c.grid(),{}),dict(gpu_seconds=2457600,cpu_stage_seconds=36008,prior_cpu_stage_seconds=8))
        self.assertLess(f['archive'],18500000000);self.assertLess(f['inclusive'],80000000000)
        text=(r.ROOT/'preserve_r1.py').read_text()
        self.assertIn('science_source=c.REPO,r1_source=Path(recovery.R1_SOURCE),r2_source=recovery.ROOT,original_control=ctx.old_control,r1_control=ctx.r1_control,r2_control=control,run=auth.run',text)
        from preserve import inventory,verify_tar
        import tarfile
        with tempfile.TemporaryDirectory() as tmp:
            root=Path(tmp);roots={}
            for label in ('science_source','r1_source','r2_source','original_control','r1_control','r2_control','run'):
            ...
            self.assertEqual(verify_tar(tar,items)['members'],7)
    ...
    def test_actual_worker_main_with_artificial_fit_and_claim(self):
        ...
        spec=c.grid()[0]
        with tempfile.TemporaryDirectory() as tmp:
            root=Path(tmp);(root/'submissions-r2').mkdir()
    ...
    def test_actual_archive_builder_all_seven_roots(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            ctx=types.SimpleNamespace(old_control=roots['original_control'],r1_control=roots['r1_control'])
            auth=types.SimpleNamespace(run=roots['run'],approval={'package_sha256':'artificial'},approval_sha='artificial')
            acceptance=roots['r2_control']/'FINAL-ACCEPTANCE.json'
            c.write(acceptance,dict(tasks=8197,episodes=8192,package='artificial',approval='artificial',run=str(auth.run),receipts=[]))
            with patch.object(p.recovery,'Context',return_value=ctx),patch.object(p.recovery,'baseline'),patch.object(p.recovery,'ROOT',roots['r2_source']),patch.object(p.recovery,'R1_SOURCE',roots['r1_source']),patch.object(c,'REPO',roots['science_source']):
                request=p.archive(auth,roots['r2_control'],acceptance)
...
if __name__=='__main__':
    began=time.monotonic();label='r2-integration-03'
    ...
    c.append(r.ROOT/'TEST-ATTEMPTS.jsonl',dict(label=label,state='finished',unix=time.time(),wall_seconds=time.monotonic()-began,passed=result.wasSuccessful(),peak_job_memory_bytes=0))
    if result.wasSuccessful():r.write(r.ROOT/'TEST-RECEIPT.json',dict(passed=True,tests=result.testsRun,tested_sources=tested,science_unchanged=True,mocked_scheduler=True,original_tasks=8197,total_attempts=8199,research_allocations=0,scientific_outcomes_read=False))
