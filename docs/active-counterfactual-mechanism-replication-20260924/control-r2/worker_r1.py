...
def main():
    ...
    ctx=recovery.Context(os.environ['ACVM_R2_APPROVAL']);c.require(a.approval==str(ctx.worker_approval) and a.run==str(ctx.run),'Recovery worker invocation');auth=ctx.auth;spec=next(j for j in c.grid() if j['key']==a.task)
    ...
    try:
        claim=auth.run/'submissions-r2'/(spec['key']+'.json')
