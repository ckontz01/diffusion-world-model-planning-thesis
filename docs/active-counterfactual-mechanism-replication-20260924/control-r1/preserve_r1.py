...
def archive(auth,control,acceptance):
    ...
    try:
        ctx=recovery.Context(control/'EXECUTION-APPROVAL.json');recovery.baseline(ctx);roots=dict(science_source=c.REPO,recovery_source=recovery.ROOT,original_control=ctx.old_control,recovery_control=control,run=auth.run)
