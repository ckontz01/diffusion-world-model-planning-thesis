...
def main():
    ...
    ctx=r.Context(os.environ['ACVM_R1_APPROVAL'])
    ...
    code,failures=bounded_child(command,[ctx.run/'logs'/(a.task+'.'+s) for s in ('out','err')],spec['seconds']-10)
    if failures:r.write(ctx.run/'logs'/(a.task+'.SUPERVISOR-FAILURE.json'),dict(errors=failures,no_retry=True))
    return code
